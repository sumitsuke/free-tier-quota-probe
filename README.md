...
- **Wall — pinned to 500 MB (decimal), on two accounts.** Bulk fill stalls ~499.9 MB; looping single-row inserts pins the ceiling at `meta.size_after = 499,994,624` (= 500,000,000 − 5,376; `122,069 × 4096` pages), after which the next write returns `HTTP 400 / 7500 / "Exceeded maximum DB size"`. Reproduced identically on two accounts (served from KIX/Osaka and NRT/Tokyo).
...
- **`CREATE INDEX` doubles storage → it hits the wall on its own.** On a 300 MB control (below the wall), `CREATE INDEX` failed with `7500 "Exceeded maximum DB size"`, *not* memory. Deleting the control to 80 MB and re-indexing showed why: `size_after` went `84,029,440 → 167,923,712` — **×2.00** — because the index carries a copy of the blob column. So it succeeds at 80 MB (→160 MB), fails at 300 MB (→~600 MB > wall), and OOMs (`SQLITE_NOMEM`) only *at* the wall. Practical limit on Free: index ≤ ~250 MB of data per column. (This reverses an earlier "it's just memory" note — measured; `results/index-*-d1.jsonl`.)
...
Caveats: **REST `/query` only** (Worker Binding API may differ, esp. `VACUUM`); the wall and the index-doubling were reproduced across two accounts, other ops are n=1; the account-storage wall and daily-read wall are not measured; the exact index-doubling threshold (~250 MB) isn't pinned; `size_after` behaves like a coherent logical meter (linear on DELETE, ×2 on index) but the docs don't confirm physical vs logical. Raw captures in [`results/`](./results); frozen predictions + follow-up amendment in [`PREREGISTRATION.md`](./PREREGISTRATION.md).
